...
description: "It's a fascinating world full of algorithms, all of which have simple origins and lucid explanations. Let's see if you can get inspired enough to code your own neural networks."
pubDatetime: 2021-06-16T00:00:00+05:30
draft: false
tags:
  - Basics
  - Neural Network
legacyPath: /basics/Neural-Nets-From-Scratch/
---

# Neural Networks

![A fully connected neural network](/assets/blog/neural-networks/network-overview.png)
...
_" but machines are idiots..if they could learn from experience....hmmm...what learns from experience...oh us!!!! Let's just try and replicate our brains then!!!!"_

And that was that! These neural networks are very simplified representations of what goes on in our brains and how we learn from experiences. A meticulous connection of millions of neurons (just like those circles) and millions of pathways (just like those lines), which fire and interact amongst themselves. It's fascinating, isn't it, to see where the inspiration comes from? We have come a long way since the 1950s with better algorithms, but even with the best hardware available on Earth, we are nowhere near reproducing what you have in your skull. There is a world of history and biology that I would have shared with you, and probably will write about soon, but I do encourage you to explore it on your own. You won't be disappointed, I promise you.

## Story Time!!!

![Learning to distinguish an elephant from a cat](/assets/blog/neural-networks/learning-example.png)
...
Let's segment out the basic unit of the neural net and see exactly what it does. It's this one right here, also known as a **perceptron**. It takes a vector of inputs. What might that vector be? In the context of the previous example, it's the vector of features.

![A perceptron receiving four input features](/assets/blog/neural-networks/perceptron.png)

**X_input = [ 4, large, tusks, teeth ]**

But the computer cannot understand such a representation. What it understands are numbers, tonnes and tonnes of numbers, and it will happily keep calculating them without fatigue. So, suppose we make a table like this:
...
In the above example, 2 in the colour column means brown and 1 means grey, like the indices we have in maps. So our X_input from the previous example is **[ 4, 100, 2, 4]**. We then feed it into the perceptron. Let's think a bit logically now. We have 4 numbers with us. The plan is that we do some operation on these numbers to get a single value. If that value is above a threshold value, the features correspond to a cat or an elephant. So, what operation can make that happen? Addition? It would make sense. The size of a cat would have to be realistically 1 if the size of an elephant is 100. That in itself could create a very big separation: the sum for a cat would average around 10-20, while that for an elephant would be 100-115. Similarly, think about other operations that we could do with these numbers, and that will be your very own algorithm!
...
Just like in our everyday lives some jobs have more priority than others, the size of the animal should have a higher priority than more generic features such as the number of feet. That is because size has a greater contribution to the final decision than the other factors. Both cats and elephants have 4 feet (barring exceptions), so just telling a kid about the 4 feet won't help him distinguish an elephant. Ponder it for a bit! One way we can assign "priorities" is by multiplying these features by weights; that is, we take the weighted sum of 1 * 4 + 4 * 100 + 1 * 2 + 1 * 4. Of course, the threshold is going to change, but the decisions we make should have more real-life significance. Similarly, some factors should have lower weights than what we have chosen right now. You can probably think of countless examples in which you would want to modify those weights. And this, people, is the **feedforward part**.
...
![equation](https://latex.codecogs.com/gif.latex?%5Clarge%20%7B%5Ccolor%7BDarkGreen%7D%20%5Cbegin%7Bbmatrix%7D%204%20%26%20100%20%26%202%20%26%204%20%5Cend%7Bbmatrix%7D%7D%20%5Ccdot%20%7B%5Ccolor%7BRed%7D%5Cbegin%7Bbmatrix%7D%201%5C%5C%204%5C%5C%201%5C%5C%201%20%5Cend%7Bbmatrix%7D%7D%20%3D%20%7B%5Ccolor%7BBlue%7D%5Cbegin%7Bbmatrix%7D%201*4%20+%204*100+%201*2+%201*4%20%5Cend%7Bbmatrix%7D%20%7D%20%3D%20%7B%5Ccolor%7BBlue%7D%5Cbegin%7Bbmatrix%7D%20410%20%5Cend%7Bbmatrix%7D%7D)
...
![equation](https://latex.codecogs.com/gif.latex?%5Clarge%20%5Csum%20X_%7Binput%7D%20.%20W%5E%7BT%7D%20+%20b)
...
![A linear decision boundary between two classes](/assets/blog/neural-networks/linear-separation-example-1.png)
![Points separated by a learned line](/assets/blog/neural-networks/linear-separation-example-2.png)
...
There is, however, just a little bit of work left, and that little something is known as an **activation function**.
...
![Graph of the sigmoid activation function](/assets/blog/neural-networks/sigmoid.png)
...
For the elephant-and-cat classification example, we did have the labels ready, didn't we? While asking the child for the correct answers, we should know the true labels ourselves. Then we can compare. Similarly, the loss function or error is:

![Loss function equation](/assets/blog/neural-networks/loss-function.png)
...
![Gradient descent moving toward lower error](/assets/blog/neural-networks/gradient-descent.png)
...
![Backpropagation update equation](/assets/blog/neural-networks/backpropagation.png)
...
And you should see the error decreasing over time and the accuracy increasing. But this one perceptron can only draw a straight line, which we have already discussed is impractical for most real-world data distributions. Introducing multiple perceptrons should give better boundaries.
...
![A single-layer network with three inputs and two outputs](/assets/blog/neural-networks/single-layer-network.png)
...
Below is the code for an MLP, which I encapsulated within a class for better modularity. The code is very similar, so try to find the similarities first. We have the same backpropagation and feedforward algorithms. The only difference is that we need to take care of more weights and biases, hence the extra loops. The same things are calculated and appended to extra lists to keep track of them.
